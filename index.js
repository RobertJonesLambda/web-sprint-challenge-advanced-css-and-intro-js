...
let dough = artists.splice([8].name, "Vincent Van Gough")
...
*/
 function getArtistByIndex(array, index) {
      let artistStatment = `The artist at ${index} is ${artists[0].name}`
   }
...
function get20s(arr, index){
    //parseInt  to convert years to integers
    for (let i=0; i < arr.length; i++){
      arr[i] = parseInt(arr[i], "years")
    }
    //for loop and if to find artists active
    for (let i=0; i < arr.length; i++){
       if (arr[i] >= )
    }  
